...
export default function ProductDetailClient({
  ...
  // The server already resolved the product (and 404'd if missing), so this
  // page never needs a loading/blank state. Once the live catalogue loads
  // client-side, swap in the freshest copy in case an admin just edited it.
  const { products: list, loaded } = useProducts();
  const product = (loaded && getProductById(initialProduct.id, list)) || initialProduct;
  const related = loaded ? getRelatedProducts(product.id, 4, list) : initialRelated;
      ...
      <section className="max-w-7xl mx-auto px-4 py-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div>
            ...
            <a href="#reviews" className="flex items-center gap-3 mb-4 w-fit">
              ...
              <span className="text-xs text-[#6b5d4c] hover:underline">
